...
pub fn generate_to<W: Write>(dest: &mut W, message_id: &str, num_txns: u32, seed: u64) {
    ...
    writeln!(dest, r#"<Document xmlns="urn:iso:std:iso:20022:tech:xsd:pain.001.001.09">"#).unwrap();
    writeln!(dest, r#"  <CstmrCdtTrfInitn>"#).unwrap();
    writeln!(dest, r#"    <GrpHdr>"#).unwrap();
    writeln!(dest, r#"      <MsgId>MSG-{message_id}</MsgId>"#).unwrap();
    writeln!(dest, r#"      <CreDtTm>2026-04-25T04:00:00+02:00</CreDtTm>"#).unwrap();
    ...
    writeln!(dest, r#"      <Dbtr><Nm>DEBTOR CORE HOLDINGS INC</Nm></Dbtr>"#).unwrap();
    writeln!(dest, r#"      <DbtrAcct><Id><IBAN>DE89370400440532013000</IBAN></Id><Ccy>EUR</Ccy></DbtrAcct>"#).unwrap();
    writeln!(dest, r#"      <DbtrAgt><FinInstnId><BICFI>DEUTDEFFXXX</BICFI></FinInstnId></DbtrAgt>"#).unwrap();
    ...
    for idx in 0..num_txns as usize {
        ...
        writeln!(dest, r#"          <EndToEndId>E2E-{message_id}-{txn:06}</EndToEndId>"#).unwrap();
        writeln!(dest, r#"        </PmtId>"#).unwrap();
        writeln!(dest, r#"        <Amt><InstdAmt Ccy="EUR">{amt:.2}</InstdAmt></Amt>"#).unwrap();
        writeln!(dest, r#"        <CdtrAgt>"#).unwrap();
        writeln!(dest, r#"          <FinInstnId><BICFI>{bic}</BICFI></FinInstnId>"#).unwrap();
        writeln!(dest, r#"          <FinInstnId><Nm>BANK {txn:06}</Nm></FinInstnId>"#).unwrap();
        writeln!(dest, r#"        </CdtrAgt>"#).unwrap();
        ...
        writeln!(dest, r#"          <Ustrd>Payment INV-{txn:08} — Ref: {message_id}</Ustrd>"#).unwrap();
        writeln!(dest, r#"          <Strd><CdtrRefInf>"#).unwrap();
        writeln!(dest, r#"            <Tp><CdOrPrtry><Cd>SCOR</Cd></CdOrPrtry></Tp>"#).unwrap();
...
/// Convenience wrapper that returns a `String`.  Only suitable for
/// small payloads — use [`generate_to`] when streaming to disk.
pub fn generate(message_id: &str, num_txns: u32, seed: u64) -> String {
    let mut buf = Vec::with_capacity(num_txns as usize * 600);
    generate_to(&mut buf, message_id, num_txns, seed);
